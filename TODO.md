...
- [x] 提示词区域监听检查器和窗口尺寸变化。
- [ ] 用干净 ComfyUI 进程确认 `/object_info` 已切换到新 schema。
- [ ] 用真实 `/prompt` 提交一次当前目标镜头执行并确认无 validation error。
...
- [ ] 顶部统一显示“当前项目”，不区分单镜 / 多镜模式。
- [ ] 显示当前目标镜头。
- [ ] 显示本次执行镜头列表。
- [ ] 在 Studio 中增加“保存与命名”二级页面。
- [ ] 显示输出目录预览。
- [ ] 显示文件名预览。
- [ ] 底部轨道排序后立即刷新前镜关系。
- [ ] 轨道显示已生成 / 未生成 / 当前目标状态。
...
- [ ] Studio 素材库替代 ImageAsset / VideoAsset / AudioAsset Manager。
- [ ] Studio 文档库替代 DocumentAsset / DocumentManager。
- [ ] 删除重复 UI 的网页注入代码。
- [ ] 保留底层服务函数，不删除真实生成、保存、拼接能力。
...
- [ ] 检查 `ComfyUI-KJNodes` 的实时预览能力。
- [ ] 缺失依赖时区分“当前模式必需”和“可选功能”。
