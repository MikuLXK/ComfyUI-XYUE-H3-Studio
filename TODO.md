...
>
> 真实 GPU 生成、素材尾帧和 Motion Context 的设备验收由用户在自己的 ComfyUI 实例中执行；代码、接口和静态验证已完成。
...
- [x] 增加“随机 / 增加 / 减少 / 固定”种子模式状态。
- [x] 随机模式运行前生成新 seed。
- [x] 沿用模式复用当前镜头最近 seed。
- [x] 在报告中写入真实使用 seed。
- [x] 种子输入区明确显示本次/上次实际种子。
- [x] 种子模式支持随机、增加、减少、固定。
- [ ] 在历史记录中显示每次重试的 seed。
...
- [x] 清空 Studio 内嵌示例提示词和镜头名称，保留生成参数默认值。
- [x] 主页面截图使用标准 16:9 比例并随 README 发布。
- [x] 新项目第一镜默认命名为 `镜头01`。
- [x] 保存设置中的重名策略显示中文选项。
- [x] 全屏快速引用列表挂载到 body 并在 fullscreenchange 后重新定位。
- [x] 快捷地址 `/xyue-h3` 直接读取 input/output 素材库。
- [x] 启用素材随动态执行配置提交到 Ref2VA。
...
- [x] 只提交当前有效代码、文档和导出示例。
- [x] 最终检查仓库中没有旧模板运行依赖。
- [x] 重新打包 `XYUE_H3_Studio_纯净包.zip` 并清理缓存字节码。
