...
独立的 MiniMax H3 本地生成工作室节点包。所有节点使用 `XYUE_H3_` 前缀，分类为 `XYUE/H3 Studio`，不依赖旧 `manga_studio_nodes`。
...
工作流已接入当前环境提供的完整 H3 加速链：

`XYUE_H3_LoRASelector -> KJ H3 Chunk FeedForward -> KJ H3 Memory Efficient SageAttention -> KJ H3 Low VRAM Attention -> TESpeedMiniMaxH3 -> Sol-Attn -> XYUE_H3_AccelerationController -> XYUE_H3_Generator`

- T2VA、I2VA、FL2VA、L2VA 使用独立的 `XYUE_H3_LoRASelector`；节点会动态读取 ComfyUI 的 LoRA 目录，并默认选择匹配 MiniMax H3 的 8 步 LoRA，也可选择“不使用 LoRA”直接旁路。
- `XYUE_H3_AccelerationController` 是完整加速链的总开关，默认关闭；关闭时采用懒执行并直接选择原始模型，不执行 LoRA、注意力或 TE-Speed 分支，开启后才使用完整加速模型。
- 两个短剧模板在全局生成控制器下方提供 `XYUE_H3_GlobalLoRAManager`，一次连接三个阶段的 LoRA 节点；关闭全局 LoRA 后，各阶段恢复使用自己的 LoRA 选择。多参考模式默认不套用 FL2VA LoRA，只有显式开启“允许用于多参考模式”后才会应用。
- 全部模板默认时长 5 秒、视频步数 12、音频步数 12，种子在每次生成后自动随机。多段默认使用 TE-Speed 的 `8-step LoRA` 模式；使用 12 步时应先做一次本机结果对照，若该 TE-Speed 版本不自动适配长轨迹，就把阶段步数改为 8，或将 TE-Speed 模式改为 `standard`。全局控制器开启时统一覆盖所有启用阶段；关闭时每个“阶段生成配置”节点独立控制分辨率、时长、视频步数、音频步数、调度器和种子。
- Ref2VA 不加载 FL2VA 专用 LoRA，使用 `TESpeedMiniMaxH3` 的 `standard` 模式，避免把首尾帧蒸馏权重错误应用到多参考模型。
- 工作流采用 KJNodes 的 H3 专用优化：分块前馈降低 SwiGLU 峰值显存，H3 Memory Efficient SageAttention 负责 H3 自注意力优化，H3 Low VRAM Attention 以 4 个 head group 分块并尽早释放中间张量。Sol-Attn 默认参数来自本机 TE 参考工作流，属于实验性稀疏注意力优化。
- TE-Speed 的 `device` 默认 `auto`，缓存控制值为 `0.08`、处理区间 `0.1–0.9`、MCS 为 `2`；可在节点中按显存和质量需求调整。
- 如果改用 4-step LoRA，应同时把 TE-Speed 模式切换为 `4-step LoRA` 并将采样步数设为 8 或更低；不要把 FL2VA LoRA 接到 Ref2VA。
...
多段短剧流程支持最多 5 条横向阶段轨道：9 张图片、3 个音频和 3 个视频素材槽进入第一阶段；未启用且未上传的槽位不会参与验证或执行。每一段生成后都先由阶段检查点节点独立保存，再由续接选择器输出实际采用的视频；任一后续阶段失败都不会丢失此前视频，也可以在对应续接选择器中读取已保存文件继续。五段视频完成面板统一负责阶段视频展示、最终拼接、保存和预览，六项预览按上 3 下 3 顺序提供。多段共享一个全局生成控制器，默认每段 5 秒、视频 12 步、音频 12 步；关闭总开关后，各阶段配置可分别修改。普通多段和全程多参考工作流都内置“XYUE_多段云端配置”和“XYUE_五段视频完成面板”节点。
